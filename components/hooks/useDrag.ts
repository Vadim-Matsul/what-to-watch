...
export const useDrag = (ref: React.MutableRefObject<HTMLElement>, should: boolean, movieId: number) => {
  if (!should) { return; }
  ...
  useEffect(() => {
     ...
     */
    const listenerDragOver = (evt: DragEvent) => {
      evt.preventDefault();
    ...
    const listenerDragStart = (evt: DragEvent) => {
      const target = evt.currentTarget as HTMLElement;
      handleChangeStartId(movieId)
      target.className = 'drag-start'
    }

    const listenerDragEnd = (evt: DragEvent) => {
      const target = evt.currentTarget as HTMLElement;
    ...
    const listenerDragLeave = (evt: DragEvent) => {
      if (startId == movieId) return;
      const target = evt.currentTarget as HTMLElement;
    ...
    const listenerDragDrop = (evt: DragEvent) => {
      const target = evt.currentTarget as HTMLElement;
      target.className = 'small-movie-card catalog__movies-card'
